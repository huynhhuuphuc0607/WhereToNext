...
public class CollegeListAdapter extends ArrayAdapter<College> {
    ...
    public View getView(int pos, View convertView, ViewGroup parent)
    {
        ...
        try {
            InputStream is = am.open(selectedCollege.getImageName());
            Drawable drawable = Drawable.createFromStream(is,selectedCollege.getName());
            collegeListImageView.setImageDrawable(drawable);
        } catch (IOException e) {
            e.printStackTrace();
        }


        view.setTag(selectedCollege);
        return view;
    }
}
